...
function displayEndGameUI() {
    ...
    layer.msg(datacollect, {
        time: 0 //不自动关闭
        , btn: ['确定', '分享'], area: ['80%', '300px'], btnAlign: 'c', shade: 0.8
        , yes: function (index) {
            layer.close(index);
            layer.confirm('<p>是否上传DNA，融合进族群？<p>', {
                type: 0,
                btn: ['确认', '取消'], anim: 0, btnAlign: 'c', shade: 0.8, closeBtn: 0, area: ['80%', '300px'] //按钮
            }, function () {
                layer.msg('DNA融合中...');
                savecell();
            }, function () {
...
function callapoptosis() {
    ...
    layer.msg(datacollect, {
        time: 0 //不自动关闭
        , btn: ['确定', '取消'], area: ['80%', '300px'], btnAlign: 'c', shade: 0.8
        , yes: function (index) {
...
//            listener: inheritancecallback
        ...
        layer.msg('获取遗传信息中...',{shade:0.8,time:30000,shadeClose:true});
    ...
    nebPay.queryPayInfo(serialNumber, {callback: callbackUrl})   //search transaction result from server (result upload to server by app)
        .then(function (resp) {
            ...
            if (formal.msg == 'success') {
                ...
                clearInterval(close);
                layer.msg('放弃遗传物质读取,请安装NAS钱包并保持网络通畅');
            }
...
//    setTimeout(function(){main();},800);
    ...
    if (cellno > 0) {
        ...
        layer.msg('<img src=\"img/btn4l.png\" height=\"120\" width=\"120\" id=\"jinhua\"></img><br>' + '<div>变异失败</div><br>至少需要1个细胞才能变异', {
            time: 1000 //不自动关闭
            , anim: 6, btnAlign: 'c', shade: 0.8, closeBtn: 0, area: ['80%', '300px'], offset: 't'
...
function worldSearch(resp) {
    ...
    layer.msg(datacollect, {
        ...
        yes: function () {
            ...
            var sharepicrandom = randomize(1, 9);
            var backgroundim = '  <link rel=\"stylesheet\" href=\"css/card.css\"><div class=\"paper\"><img class=\"poster\" src="img/p' + sharepicrandom + '.png"/> <h2>世界-最终评价</h2>  <h1>' + worldtitle + '</h1> <p>第 ' + result.id + ' 世界里细胞存活了 ' + result.day + ' 天.共获得了 ' + result.totoalscore + ' 分。把这个世界截图分享给你的朋友吧</p>  <img class=\"qrcode\" src=\"img/share.png\"/></div>'
            layer.msg(backgroundim, {
                time: 0, //不自动关闭
                shadeClose: true,
                area: "70%",
