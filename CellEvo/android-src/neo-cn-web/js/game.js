...
function totalcellSearch(resp) {
    const result = resp.result    ////resp is an object, resp.result is a JSON string

    inputid = parseInt(result);
    console.log(inputid);
    const nextid = parseInt(inputid) + 1;
    const args = `["(str)nextid", "(integer)${nextid}", "(str)cellno", "(integer)${cellno}", "(str)adaption", "(integer)${adaption}", "(str)surviveability", "(integer)${surviveability}","(str)division", "(integer)${division}" ,"(str)environment", "(integer)${environment}","(str)day", "(integer)${day}" ,"(str)totoalscore", "(integer)${totoalscore}","(str)finaltitle", "(str)${finaltitle}"]`;
    makerawtransaction(args);
